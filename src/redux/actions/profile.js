...
export const uploadImage=(formdata)=>async (dispatch)=>{
    try {
        ...
        const {data}= await axios.put(`${server}/upload`,formdata,{
            headers:{
                "Content-Type":"multipart/form-data",
            },
            withCredentials:true,
        })
        dispatch({type:"uploadSuccess",payload:data.message})
    } catch (error) {
        dispatch({type:"uploadFail",payload:error.response.data.message});
    }
}
